# YOLOv3 transfer learning model for Edge Impulse

This repository is an example on how to [bring your own model](https://docs.edgeimpulse.com/docs/adding-custom-transfer-learning-models) into Edge Impulse. This repository is using YOLOv3 (an object detection model), but the same principles apply to other transfer learning models.
...
1. Convert the training data / training labels into YOLOv3 format using [extract_dataset.py](extract_dataset.py).
1. Train YOLOv3 model (using https://github.com/ultralytics/yolov3).
1. Convert the YOLOv3 model into TFLite format.
1. Done!

> **Note on epoch count:** YOLOv3 might take a while to converge, especially on large images. Play around with epoch count, or lower the resolution of your input until you have something that works.
...
10. Run the container to test the script (you don't need to rebuild the container if you make changes):
    ...
    ```
    $ docker run --shm-size=1024m --rm -v $PWD:/scripts yolov3 --data-directory data/ --epochs 30 --learning-rate 0.01 --out-directory out/
    ```
